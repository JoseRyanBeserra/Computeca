...
- [ ] T003 Extrair de `front/src/pages/MaterialDetailPage.tsx` (função `handleOpenPdf`) a escolha entre as três rotas de URL pré-assinada para uma função exportada em `front/src/features/materials/api/materialsApi.ts`, preservando exatamente a regra atual: `PROFESSOR`/`ADMIN` → `getReviewPresignedUrlRequest`; material `APPROVED` → `getPublicPresignedUrlRequest`; demais casos → `getMaterialPresignedUrlRequest`
- [ ] T004 Criar `front/src/features/materials/hooks/useMaterialFileUrl.ts` com `useQuery`, chave **`['material-file-url', materialId]`** — independente da chave do material, para que a falha de uma não contamine a outra. Assinatura `(materialId: string, materialStatus: MIStatus, enabled?: boolean)`, seguindo o padrão de `useAllMaterials` (depende de T003)
...
- [ ] T011 [P] [US1] Teste em `front/src/components/PdfPreview.test.tsx` confirmando que, com a URL disponível, um elemento `<object>` com `type="application/pdf"` é renderizado apontando para a URL obtida
...
- [ ] T018 [US1] Inserir `<PdfPreview>` em `front/src/pages/MaterialDetailPage.tsx`, entre os metadados e as ações, passando `handleOpenPdf` como `onOpenFullscreen` para que exista **um único** caminho de abertura em tela cheia (depende de T015)
...
- [ ] T026 [US2] Em `front/src/components/PdfPreview.tsx`, preencher o **conteúdo de fallback** do `<object>` com explicação e chamada para abrir em tela cheia — cobre nativamente o navegador sem suporte a PDF embutido, sem depender de detecção por JavaScript (depende de T016)
