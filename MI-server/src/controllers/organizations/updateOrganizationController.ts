// src/controllers/organizations/updateOrganizationController.ts
import type { FastifyRequest, FastifyReply } from 'fastify'
import { updateOrganizationService } from '../../services/organizations/updateOrganizationService'
...
const ctx = 'updateOrganizationController'
...
export async function updateOrganizationController(
    ...
    payload: [
      {
        ...
        content: { method: request.method, url: request.url, orgId: params.orgId, body: { name: body.name, description: body.description } },
      },
    ],
  ...
  try {
    ...
    await createInspectionLog({
      ...
      payload: [
        { title: 'DB - Organização atualizada', content: { id: org.id, name: org.name } },
    ...
    await createInspectionLog({
      ...
      payload: [
        {
          title:   'Erro - Atualização de organização',
          content: {
